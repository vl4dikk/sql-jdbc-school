...
public class StudentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (type.equals("AssignToCourse")) {
			printAssignToCourse(out);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("course") != null) {
			...
			request.getSession().setAttribute("STUDENT_ID_DELETE", request.getParameter("STUDENT_ID_DELETE"));
		} else if (request.getParameter("COURSE_ID_DELETE") != null) {
			String studentId = String.valueOf(request.getSession().getAttribute("STUDENT_ID_DELETE"));
			LinkedList<String> answer = controller.deleteFromCourse(studentId,
					request.getParameter("COURSE_ID_DELETE"));
			for (String res : answer) {
				out.print(res);
			}
		} else if (request.getParameter("STUDENT_ID_ASSIGN") != null) {
			printAssignToCourse2(out);
			request.getSession().setAttribute("STUDENT_ID_ASSIGN", request.getParameter("STUDENT_ID_ASSIGN"));
		} else if (request.getParameter("COURSE_ID_ASSIGN") != null) {
			String studentId = String.valueOf(request.getSession().getAttribute("STUDENT_ID_ASSIGN"));
			LinkedList<String> answer = controller.assignToCourse(studentId, request.getParameter("COURSE_ID_ASSIGN"));
			for (String res : answer) {
				out.print(res);
			}
		}
	...
	private void printAssignToCourse(PrintWriter out) {
		out.print("<form method = post>");
		out.print("<br> <label for=\"GET-name\">Enter student id :</label><br>");
		out.print("<br> <input id=\"GET-name\" type=\"text\" name=\"STUDENT_ID_ASSIGN\">");
		out.print("<input\r\n" + "			type=\"submit\" value=\"Next\">");
		out.print("</form>");
	...
	private void printAssignToCourse2(PrintWriter out) {
		out.print("<form method = post>");
		out.print("<br> <label for=\"GET-name\">Enter course id :</label><br>");
		out.print("<br> <input id=\"GET-name\" type=\"text\" name=\"COURSE_ID_ASSIGN\">");
		out.print("<input\r\n" + "			type=\"submit\" value=\"Assign\">");
		out.print("</form>");

		LinkedList<String> answer = controller.getAllCourses();
		for (String res : answer) {
			out.print(res);
		}
	}
